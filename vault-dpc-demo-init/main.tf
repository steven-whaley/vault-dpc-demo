...
resource "hcp_hvn" "vault-dpc-demo-hvn" {
  hvn_id         = "vault-dpc-demo-hvn"
  cloud_provider = "aws"
  region         = var.region
...
resource "hcp_vault_cluster" "vault-dpc-demo-cluster" {
  cluster_id      = "vault-dpc-demo-cluster"
  hvn_id          = hcp_hvn.vault-dpc-demo-hvn.hvn_id
  tier            = "dev"
...
resource "hcp_vault_cluster_admin_token" "tfc-vault-token" {
  cluster_id = hcp_vault_cluster.vault-dpc-demo-cluster.cluster_id
}
...
resource "tfe_variable" "vault_addr" {
  key          = "vault_addr"
  value        = hcp_vault_cluster.vault-dpc-demo-cluster.vault_public_endpoint_url
  category     = "terraform"
  variable_set_id = tfe_variable_set.vault-dpc-demo.id
}
